...
self_checks() {
  for m in src.shared.nethttp src.shared.detector src.shared.predictor \
           ...
           src.shared.audit src.shared.scoreboard src.shared.explain src.shared.workflow; do
    "$PY" -m "$m" || return 1
  done
}

smoke() {
  PYTHONIOENCODING=utf-8 "$PY" -W ignore - <<'PYEOF'
...
assert c.get("/api/capabilities").json()["keys_required"] == []
