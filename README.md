...
| **Engine 2 — Association + Attribution** | Innovation | Ranks ATT&CK techniques associated in tactic-sorted group/campaign profiles. Chronological next-move output is disabled until an independent timeline benchmark beats temporal baselines; actor ranking uses transparent profile retrieval and abstains from naming an actor until independently calibrated thresholds exist. |
