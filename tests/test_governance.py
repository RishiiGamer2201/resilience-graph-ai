...
def test_rotation_archives_and_links_generations_across_restart(tmp_path):
    ...
    result = c.rotate(
        actor="Root Administrator", role="admin", reason="end of incident shift",
        subject="root@soc", display_name="Root Administrator",
    )
    ...
    assert c.records()[0]["subject"] == "root@soc"
    assert c.records()[0]["display_name"] == "Root Administrator"
