...
def _map_agent_graph(agent_summary: dict, base_graph: dict) -> dict:
    ...
    graph = {
        **base_graph,
        ...
        "nodes": nodes,
        # Current React graph reads `edges`; 3D graph consumers usually read `links`.
        # Carry both names from the same mapped edge list.
        "edges": links,
        "links": links,
        "n_nodes": len(nodes),
        "n_edges": len(links),
...
def _map_agent_bundle(bundle: dict, agent_summary: dict) -> dict:
    ...
    if narrative:
        bundle.setdefault("overview", {}).setdefault("active_incident", {})["summary"] = narrative
        bundle["overview"]["incident_narrative"] = narrative
        bundle.setdefault("report", {})["summary"] = narrative
        bundle["report"]["agent_narrative"] = narrative

    if ranked_chains:
        bundle.setdefault("incident", {})["ranked_chains"] = ranked_chains
        bundle["incident"]["active_threats"] = ranked_chains
        bundle.setdefault("overview", {})["active_threats"] = ranked_chains[:5]
        bundle.setdefault("report", {})["ranked_chains"] = ranked_chains

    bundle["graph"] = _map_agent_graph(agent_summary, bundle.get("graph", {}))

    if technique_mapping:
        bundle.setdefault("threat_intel", {})["mapping"] = technique_mapping
        bundle["threat_intel"]["validated_technique_ids"] = [m["technique_id"] for m in technique_mapping]
        bundle["threat_intel"]["note"] = (
            "ATT&CK techniques are emitted by the Intelligence agent and validated by "
            "the orchestrator's schema/evidence gates."
        )
