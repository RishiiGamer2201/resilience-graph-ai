...
- [x] 🟡 **E2.2b Manual CERT-In sequences** — ✅ **DONE (teammate, verified 4/4, 2026-07-16).** 4 REAL CERT-In advisories, report-ordered, `verified:true`: WhatsApp→RMM loader (CICA-2026-3534), RTO/eChallan Android banking trojan (CICA-2026-3492), npm/PyPI supply-chain (CICA-2026-3493), Fortigate/SSL-VPN credential-stuffing "FortiBleed" (CICA-2026-3534). Each cites its exact advisory URL.
  - **Acceptance:** ✅ 4 verified sequences in `data/manual/cert_in_sequences.json`; `reports/sequences.md` shows 4/4 verified.
  - **⭐ Enabled a real dataset expansion:** the Android trojan sequence is **Mobile ATT&CK** (T1660/T1636/T1638…), which our Enterprise+ICS lookups didn't have. Added the **mobile-attack STIX bundle** → `attack_lookups.pkl` now **918 techniques** (was 794); embeddings regenerated on CPU (GPU was busy) → same-tactic cos 0.412. (Teammate's `T1476` dropped — deprecated in current Mobile ATT&CK.)
  - **KEY FINDING (updated):** shipped Markov on the verified manual set = **top-3 10.0%** (30 prediction points, 6 OOV) vs **36.5%** on the kill-chain-ordered auto set → real reported orderings are harder; the auto number is partly ordering-driven. Anti-circularity: Markov **5.2× the kill-chain baseline**. Prediction stays a supporting feature; lean the pitch on Engine 1 + attribution. See `reports/prediction_eval.md`.
